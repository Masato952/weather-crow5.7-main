...
private:
  ...
  String openWeatherUrlQuery;//use city or lat, lon
  ...
  DynamicJsonDocument weatherApiResponse = DynamicJsonDocument(JSON_CAPACITY);//forecast weather info
  DynamicJsonDocument weatherCrtResponse = DynamicJsonDocument(JSON_CAPACITY);//current weather info
  bool ledState = false;
  ...
  void connectToWiFi()
  {
    WiFi.begin(cfgs.wifi_ssid, cfgs.wifi_password);
  ...
  String constructIPinforEndpointUrl()
  {
    return "https://ipinfo.io/json?token=" + cfgs.ipinfo_apikey;
  }

  String constructweathertUrl()
  {
    return "https://api.openweathermap.org/data/2.5/weather?" +
           openWeatherUrlQuery +
           "&appid=" + cfgs.openweather_apikey +
           "&units=" + UNITS;
  }  
  ...
  {
    // 直接读取 weatherCrtResponse 对应的字段
    weatherInfo.weather = weatherCrtResponse["weather"][0]["main"].as<String>();
    weatherInfo.icon = weatherCrtResponse["weather"][0]["icon"].as<String>();
    weatherInfo.currentDateTime = weatherCrtResponse["dt"].as<long>() + weatherCrtResponse["timezone"].as<long>();
    weatherInfo.sunrise = weatherCrtResponse["sys"]["sunrise"].as<long>();
    weatherInfo.sunset = weatherCrtResponse["sys"]["sunset"].as<long>();
    weatherInfo.temperature = String(weatherCrtResponse["main"]["temp"].as<float>(), 1); // feels_like
    weatherInfo.humidity = String(weatherCrtResponse["main"]["humidity"].as<int>());
    weatherInfo.pressure = String(weatherCrtResponse["main"]["pressure"].as<int>());
...
bool parseLocationFromJson(String json)
{
  ...
  if (error)
  {
    ...
    errorMessageBuffer = "IPInfo JSON parse error: " + String(error.c_str());
    return false;
  }

  // Check if there's an error field (e.g. token 无效)
  if (doc.containsKey("error"))
  {
    const char *title = doc["error"]["title"] | "Unknown error";
    const char *message = doc["error"]["message"] | "No details.";
    errorMessageBuffer = String("IPInfo API error: ") + title + "\n" + message;
    return false;
  }
  ...
  if (loc)
  {
    ...
    if (commaIndex > 0)
    {
      cfgs.loc_lat = locStr.substring(0, commaIndex);
      cfgs.loc_lon = locStr.substring(commaIndex + 1);
      if (cfgs.loc_lat == "" || cfgs.loc_lon == "")
      {
        errorMessageBuffer = "lat and lon is null got from IP info.";
        return false;
      }
      return true;
    }
  ...
  errorMessageBuffer = "Failed to extract latitude/longitude from IP info.";
  return false;
}


  bool getIPInfo()
    // Step 1: Get IP info to determine latitude and longitude
  {
    String ipJson = httpsGETRequest(constructIPinforEndpointUrl().c_str());
    logPrintln("IP Info JSON:");
    Serial.println(ipJson);
    if (!parseLocationFromJson(ipJson)) {
      return false;
    }
  ...
  bool fetchWeatherData(const String& apiEndPoint, DynamicJsonDocument& targetDoc, uint8_t maxRetries = 3)
  {
    ...
    while (currentRetry <= maxRetries && !success)
    {
      ...
      String jsonBuffer = httpsGETRequest(apiEndPoint.c_str());
      ...
      DeserializationError error = deserializeJson(targetDoc, jsonBuffer);
      if (error)
      {
  ...
  {
    if(cfgs.sd_failed){
      errorMessageBuffer = "SD card not found. Please insert an SD card.";
      return false;  
    }
    if (cfgs.wifi_ssid.length() == 0 || cfgs.wifi_password.length() == 0) {
      errorMessageBuffer = "WiFi SSID or password not set. Please configure them in config.json.";
      return false;
    }
    connectToWiFi();
    if (WiFi.status() != WL_CONNECTED)
    ...
    if(cfgs.loc_city.length() > 0){
      openWeatherUrlQuery = "q=" + cfgs.loc_city;
    }else if (cfgs.loc_lat.length() > 0 && cfgs.loc_lon.length() > 0) {
      openWeatherUrlQuery = "lat=" + cfgs.loc_lat + "&lon=" + cfgs.loc_lon;
    }else if (cfgs.ipinfo_apikey.length() > 0) {
      if (getIPInfo())
      {
        openWeatherUrlQuery = "lat=" + cfgs.loc_lat + "&lon=" + cfgs.loc_lon;
      }
    }else{
      errorMessageBuffer = "No valid location data in config.json.";
      return false;
    }
    if(cfgs.openweather_apikey.length()==0){
      errorMessageBuffer = "No valid openweather_apikey in config.json.";
      return false;
    }
    //current weather
    String currentWeatherUrl = constructweathertUrl();
    bool currentWeatherFetched = fetchWeatherData(currentWeatherUrl, weatherCrtResponse, maxRetries);
    if (!currentWeatherFetched)
      {
      logPrintln("Failed to fetch current weather data.");
      return false;
      }

    //weather forecast
    String forecastWeatherUrl = constructApiEndpointUrl();  // 例如是 forecast API URL
    bool forecastFetched = fetchWeatherData(forecastWeatherUrl, weatherApiResponse, maxRetries);
    if (!forecastFetched)
    {
      logPrintln("Failed to fetch forecast weather data.");
      return false;
    }
  ...
  void drawForecastItem(uint16_t x, uint16_t y, JsonObject hourlyData)
  {
    ...
    long forecastTimeInt = hourlyData["dt"].as<long>() + weatherInfo.l_timezone;
    ...
    // Temperature
    memset(buffer, 0, sizeof(buffer));
    float temp = hourlyData["main"]["temp"].as<float>();//feels_like
    int tempInt = (int)(temp >= 0 ? temp + 0.5f : temp - 0.5f);//int tempInt = (int)temp;
  ...
  void drawWeatherFutureForecast(uint16_t baseX, uint16_t baseY, uint16_t length)
  {
    ...
    int forecastsToShow = length;//min(length, (uint16_t)availableHours);

    for (uint16_t counter = 0; counter < forecastsToShow; counter++)
    {

      JsonObject weather_lists = weatherApiResponse["list"][counter];

      drawForecastItem(x, y, weather_lists);
  ...
  void displayWeatherForecast()
  {
    ...
    snprintf(buffer, sizeof(buffer), "%s | %s",
         weatherInfo.city_name.c_str(),
         convertUnixTimeToDisplayFormat(weatherInfo.currentDateTime).c_str());
...
void loop()
{
  ...
  else
  {
    ...
    esp_sleep_enable_timer_wakeup(1000000ULL * 60ULL * 1);
    esp_deep_sleep_start();
  }
}
